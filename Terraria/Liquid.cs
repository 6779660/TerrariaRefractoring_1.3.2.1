	...
	public class Liquid
	{
		...
		public void Update()
		{
			...
			if (tile5.liquid > 0)
			{
				...
				{
					if (tile.liquid > 0 && tile.liquidType() != tile5.liquidType())
					{
						flag = false;
					}
					else
					{
						...
						else
						{
							...
							{
								if (Main.tile[this.x - 2, this.y].liquid > 0 && Main.tile[this.x - 2, this.y].liquidType() != tile5.liquidType())
								{
									flag3 = false;
								}
							}
				...
				{
					if (tile2.liquid > 0 && tile2.liquidType() != tile5.liquidType())
					{
						flag2 = false;
					}
					else
				...
				if (flag && flag2)
				{
					if (flag3 && flag4)
					{
						...
						else
						{
							...
							{
								if (Main.tile[this.x - 3, this.y].liquidType() != tile5.liquidType())
								{
									flag5 = false;
								}
							}
		...
		public static void LavaCheck(int x, int y)
		{
			...
			else
			{
				if (tile4.liquid > 0 && !tile4.lava())
				{
					...
					{
						if (tile4.active() && Main.tileObsidianKill[(int)tile4.type])
						{
							WorldGen.KillTile(x, y + 1, false, false, false);
							if (Main.netMode == 2)
							{
		...
		public static void DelWater(int l)
		{
			...
			else
			{
				...
				{
					if (tile3.liquid < 255 && (!tile3.nactive() || !Main.tileSolid[(int)tile3.type] || Main.tileSolidTop[(int)tile3.type]) && !Liquid.stuck)
					{
					...
					for (int i = num - 1; i <= num + 1; i++)
					{
						for (int j = num2 - 1; j <= num2 + 1; j++)
						{
							...
							if (tile5.active())
							{
								...
								{
									if (tile5.type == 60 || tile5.type == 70)
									{
										tile5.type = 59;
										WorldGen.SquareTileFrame(i, j, true);
										if (Main.netMode == 2)
